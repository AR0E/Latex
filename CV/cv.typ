...
#let showrules(settings, doc) = {
    ...
    show heading.where(
        ...
        #set align(left)
        #set text(font: settings.font-heading, size: 1em, weight: "bold")
        #if (settings.at("headingsmallcaps", default:false)) {
            smallcaps(it.body)
        } else {
            it.body
        }
        #v(-0.65em) #line(length: 100%, stroke: 0.7pt + gray.darken(70%)) // draw a line
    ]
    ...
    show heading.where(
        ...
        #set text(font: settings.font-heading, size: 1.7em, weight: "semibold")
        #if (settings.at("headingsmallcaps", default:false)) {
            smallcaps(it.body)
        } else {
            it.body
